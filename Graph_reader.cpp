	...
	namespace reader {
		...
		namespace gowalla_austin_dallas {
			...
			network read_edges(std::string edge_file) {
				...
				auto n_friends = x3::int_ >> ' ' >> x3::omit[x3::int_] >> ' ' >> ((x3::int_ >> ' ' >> x3::omit[x3::int_]) % ' ') >> x3::eol;
				...
				if (f != l) {
					throw std::runtime_error("Could only parse " +
							std::to_string(std::distance(mm.begin(), f)) + " of " +
							std::to_string(std::distance(mm.begin(), l)) +
...
BOOST_FUSION_ADAPT_STRUCT(
		gsinfmax::reader::event_location,
		(double, longitude)
		(double, latitude)
		)
