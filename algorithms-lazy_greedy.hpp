...
namespace gsinfmax { namespace algorithms {
	class lazy_greedy {
		public:
			std::unordered_map<vertex_descriptor, int> maximize_influence(std::vector<unsigned int> budgets);
			std::unordered_map<vertex_descriptor, int> maximize_influence_baseline(std::vector<unsigned int> budgets);
		...
		private:
			...
			std::unordered_map<vertex_descriptor, int> random_propagation(const std::unordered_map<vertex_descriptor, int>& s,
					const std::unordered_map<vertex_descriptor, int>& ignore = std::unordered_map<vertex_descriptor, int>());
			std::unordered_set<vertex_descriptor> random_neighbors(const vertex_descriptor user);
